...
function resolveVehicleDispatchCategory(vehicle) {
...
// shared/email-validation.ts
var COMMON_TLD_TYPOS = {
  con: "com",
  cmo: "com",
  comm: "com",
  coom: "com",
...
function validateEmailAddress(value) {
  ...
  if (normalized.length > 254 || localPart.length > 64 || localPart.startsWith(".") || localPart.endsWith(".") || localPart.includes("..") || !/^[a-z0-9.!#$%&'*+/=?^_`{|}~-]+$/i.test(localPart)) {
    return { valid: false, normalized, message: "Please enter a valid email address." };
  }
...
async function registerRoutes(app2) {
  ...
  try {
    ...
    await pool2.query("CREATE INDEX IF NOT EXISTS idx_rides_created_at ON rides(created_at DESC)");
    await pool2.query("CREATE INDEX IF NOT EXISTS idx_rides_status_created_at ON rides(status, created_at DESC)");
    await pool2.query("CREATE INDEX IF NOT EXISTS idx_rides_client_created_at ON rides(client_id, created_at DESC)");
    await pool2.query("CREATE INDEX IF NOT EXISTS idx_rides_chauffeur_created_at ON rides(chauffeur_id, created_at DESC)");
    await pool2.query("CREATE INDEX IF NOT EXISTS idx_payments_ride_created_at ON payments(ride_id, created_at DESC)");
    await pool2.query("CREATE INDEX IF NOT EXISTS idx_earnings_ride_created_at ON earnings(ride_id, created_at DESC)");
    await pool2.query("CREATE INDEX IF NOT EXISTS idx_ride_ratings_ride_created_at ON ride_ratings(ride_id, created_at DESC)");
    await pool2.query("CREATE INDEX IF NOT EXISTS idx_safety_reports_ride_created_at ON safety_reports(ride_id, created_at DESC)");
    await pool2.query("CREATE INDEX IF NOT EXISTS idx_messages_ride_created_at ON messages(ride_id, created_at DESC)");
    await pool2.query(`
  ...
  const dispatchingRideIds = /* @__PURE__ */ new Set();
  const activeVehicleLookupCache = /* @__PURE__ */ new Map();
  ...
  async function reconcileApprovedFleetCategories() {
    ...
    const resolvedByVehicleId = /* @__PURE__ */ new Map();
  ...
  void reconcileApprovedFleetCategories().catch((error) => {
  ...
  async function getEligibleChauffeursForRide(ride, options = {}) {
    ...
    const eligible = evaluations.map((result) => result.eligible).filter(Boolean);
...
If you did not request this, you can ignore this email.`,
  ...
  app2.get("/api/admin/referrals", requireAuth, requireRole(["admin"]), async (_req, res) => {
    try {
      const result = await pool2.query(`
      ...
      const referralBase = String(
        process.env.EXPO_PUBLIC_REFERRAL_LINK_BASE_URL || process.env.EXPO_PUBLIC_DOMAIN || "https://a2blift.com"
      ).replace(/\/$/, "");
      const relationships = result.rows.map((row) => {
        const referralCode = normalizeReferralCode(row.referral_code_used || row.referrer_code);
        const app3 = row.referrer_role === "chauffeur" || row.referrer_role === "driver" ? "driver" : "client";
        const totalRewards2 = Number(row.total_rewards || 0);
        return {
          referrer: {
          ...
          referralCode,
          shareUrl: referralCode ? `${referralBase}/r/${encodeURIComponent(referralCode)}?app=${encodeURIComponent(app3)}` : null,
          status: totalRewards2 > 0 ? "rewarded" : row.referral_status || "registered",
          ...
          totalRewards: totalRewards2,
          rewardCount: Number(row.reward_count || 0)
        };
      });
      const uniqueReferrers = new Set(relationships.map((item) => item.referrer.id)).size;
      const referredDrivers = relationships.filter(
        (item) => item.referred.role === "chauffeur" || item.referred.role === "driver"
      ).length;
      const rewardedReferrals = relationships.filter((item) => item.totalRewards > 0).length;
      const totalRewards = relationships.reduce((sum, item) => sum + item.totalRewards, 0);
      return res.json({
        summary: {
  ...
  async function loadVehicleSelectionContext(userId, vehicleId) {
    return pool2.query(
  ...
  app2.post("/api/vehicles/:id/select-active", requireAuth, async (req, res) => {
    try {
      ...
      void pumpUnassignedSearchingRides().catch((error) => {
  ...
  function getEarningsWeekBounds(now = /* @__PURE__ */ new Date()) {
    ...
    try {
      ...
      for (const dp of validDriverProfiles) {
        if (!driverMap.has(dp.id)) {
          ...
          const chauffeur2 = await storage.getChauffeurByUserId(dp.userId);
      ...
      if (profile.type === "driver" && !driverMap.has(profile.id)) {
        ...
        const chauffeur2 = await storage.getChauffeurByUserId(profile.userId);
      ...
      const totalPayouts = (allWithdrawals || []).filter((w) => w.status === "completed" || w.status === "approved" || w.status === "paid").reduce((sum, w) => sum + (parseFloat(w.amount) || 0), 0);
  ...
  app2.get("/api/fleet/live-locations", requireAuth, async (req, res) => {
    try {
      ...
      const ownedVehicles = await storage.getVehiclesByOwnerOperator(profile.id);
      let driverAssignedVehicles = [];
      if (profile.type === "driver") {
        const driverAssignments = await storage.getVehicleAssignments({ driverOperatorProfileId: profile.id });
        const assignedVehicleIds = driverAssignments.map((a) => a.vehicleId);
        const fetchedAssignedVehicles = await Promise.all(
          assignedVehicleIds.map((id) => storage.getVehicle(id))
        );
        driverAssignedVehicles = fetchedAssignedVehicles.filter(Boolean);
      }
      ...
      if (partnerChauffeur?.activeVehicleId) {
        ...
        if (activeV && !ownedVehicles.some((v) => v.id === activeV.id) && !driverAssignedVehicles.some((v) => v.id === activeV.id)) {
      ...
      const vehicles2 = Array.from(vehicleMap.values());
      ...
      const [assignmentsByPartner, assignmentsByDriver, assignmentsByVehicles, fleetInvites] = await Promise.all([
        storage.getVehicleAssignments({ assignedByOperatorProfileId: profile.id }),
        profile.type === "driver" ? storage.getVehicleAssignments({ driverOperatorProfileId: profile.id }) : Promise.resolve([]),
      ...
      const liveDrivers = await Promise.all(
        validDriverProfiles.map(async (dp) => {
          ...
          return {
            ...
            phone: user?.phone || chauffeur2?.phone || "",
  ...
  function resolveSouthAfricanCity(lat, lng) {
  ...
  app2.get("/api/admin/live-map", requireAuth, requireRole(["admin"]), async (req, res) => {
    try {
      ...
      for (const chauffeur2 of allChauffeurs) {
        ...
        const currentRide = allRides.find(
          (r) => r.chauffeurId === chauffeur2.id && activeStatuses.has(r.status)
        );
        const hasRecentPing = Boolean(
          chauffeur2.locationUpdatedAt && !isNaN(new Date(chauffeur2.locationUpdatedAt).getTime()) && now - new Date(chauffeur2.locationUpdatedAt).getTime() <= 15 * 60 * 1e3
        );
        const isOnline = Boolean(chauffeur2.isOnline) || hasRecentPing || Boolean(currentRide);
        ...
        const vehicleModel = vehicle?.vehicleModel || vehicle?.model || chauffeur2.vehicleModel || "";
        const plateNumber = vehicle?.plateNumber || chauffeur2.plateNumber || "";
        const carColor = vehicle?.carColor || vehicle?.color || chauffeur2.carColor || "";
        const category = vehicle?.vehicleType || vehicle?.category || chauffeur2.vehicleType || "Standard";
        ...
        cars.push({
          ...
          chauffeurId: chauffeur2.id,
          userId: chauffeur2.userId,
          driverName: chauffeur2.phone || "Driver",
          driverPhone: chauffeur2.phone || "",
          driverPhoto: chauffeur2.profilePhoto || null,
          vehicleId: vehicle?.id || chauffeur2.activeVehicleId || null,
          make: carMake,
          model: vehicleModel,
          plateNumber,
          color: carColor,
          ...
          latitude: lat,
          longitude: lng,
          heading: typeof chauffeur2.heading === "number" ? chauffeur2.heading : void 0,
          speed: chauffeur2.speed,
          isOnline: true,
          status,
          city: carCity.name,
          ...
          lastPingAt: chauffeur2.locationUpdatedAt || null,
          activeRide: currentRide ? {
            id: currentRide.id,
            status: currentRide.status,
            pickupAddress: currentRide.pickupAddress,
            dropoffAddress: currentRide.dropoffAddress,
            passengerName: currentRide.passengerName || void 0,
            price: currentRide.price
          } : null
        });
      }
      ...
      if (cityQuery !== "all" && cityQuery !== "") {
        ...
        const targetCity = SA_MAIN_CITIES.find((c) => {
          ...
          return normKey.includes(normalizedQuery) || normName.includes(normalizedQuery) || normalizedQuery.includes(normKey) || normalizedQuery.includes("portelizabeth") && c.key === "gqeberha" || normalizedQuery.includes("mbombela") && c.key === "nelspruit" || normalizedQuery.includes("joburg") && c.key === "johannesburg" || normalizedQuery.includes("pta") && c.key === "pretoria";
        });
        if (targetCity) {
      ...
      return res.json({
        ...
        center: selectedCityMeta ? { lat: selectedCityMeta.lat, lng: selectedCityMeta.lng, zoom: 12 } : { lat: -28.4793, lng: 24.6727, zoom: 6 },
        stats: {
  ...
  app2.post("/api/rides", requireAuth, async (req, res) => {
    try {
      ...
      void dispatchNextRideOffer(enrichedRide).catch((dispatchError) => {
    ...
    async (req, res) => {
      try {
        ...
        const [countResult, rowsResult] = await Promise.all([
          pool2.query(`SELECT COUNT(*)::int AS total FROM rides r ${countJoins} WHERE ${where}`, countValues),
          pool2.query(
            `SELECT
               r.id,
               r.status,
  ...
  app2.get(
    ...
    async (req, res) => {
      try {
        const tripResult = await pool2.query(
          `SELECT
             r.*,
             client_user.name AS "clientName",
             client_user.username AS "clientEmail",
        ...
        const [paymentsResult, earningsResult, ratingsResult, reportsResult, messagesResult] = await Promise.all([
          ...
          pool2.query("SELECT COUNT(*)::int AS count FROM messages WHERE ride_id = $1", [req.params.id])
        ]);
        return res.json({
          trip,
      ...
      try {
        const [rideMetricsResult, earningsMetricsResult, withdrawalMetricsResult, reportMetricsResult, allChauffeurs] = await Promise.all([
          pool2.query(`
            ...
            FROM rides
          `),
          pool2.query("SELECT COALESCE(SUM(commission), 0)::float AS commission, COALESCE(SUM(amount), 0)::float AS driver_earnings FROM earnings"),
          pool2.query("SELECT COUNT(*) FILTER (WHERE status = 'pending')::int AS pending FROM withdrawals"),
          pool2.query("SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE status = 'open')::int AS open FROM safety_reports"),
          storage.getAllChauffeurs()
        ]);
...
<body>
  ...
  app2.put("/api/admin/users/:id/email", requireAuth, requireRole(["admin"]), async (req, res) => {
    try {
      ...
      await storage.updateUser(user.id, { username: validation.normalized });
      await storage.createNotification({
        userId: user.id,
