...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  try {
    ...
    `);
    await pool.query("ALTER TABLE documents ADD COLUMN IF NOT EXISTS pay_later_application_id varchar REFERENCES pay_later_applications(id) ON DELETE CASCADE");
    await pool.query("CREATE INDEX IF NOT EXISTS idx_pay_later_status ON pay_later_applications(status)");
    await pool.query("CREATE INDEX IF NOT EXISTS idx_pay_later_transactions_user ON pay_later_transactions(user_id, created_at DESC)");
  } catch (error) {
      ...
      if (vehicleType !== vehicle.vehicleType || chauffeur.vehicleType !== vehicleType) {
        void Promise.all([
          vehicleType !== vehicle.vehicleType
            ...
            : Promise.resolve(undefined),
          chauffeur.vehicleType !== vehicleType
            ? storage.updateChauffeur(chauffeur.id, { vehicleType }).catch(() => undefined)
            : Promise.resolve(undefined),
        ]);
      }
  ...
  const RESERVATION_DISPATCH_LEAD_MS = 20 * 60 * 1000;
  ...
  async function pumpUnassignedSearchingRides() {
    ...
    try {
      ...
      for (const ride of allRides) {
        // Promote reservations to "searching" ~20 minutes before pickup time
        if (ride.status === "reserved" && (ride as any).scheduledFor) {
          const scheduledMs = new Date((ride as any).scheduledFor).getTime();
          if (Number.isFinite(scheduledMs) && now >= scheduledMs - RESERVATION_DISPATCH_LEAD_MS) {
            const promoted = await storage.updateRide(ride.id, { status: "searching" } as any).catch(() => null);
            if (promoted) {
              (ride as any).status = "searching";
              try {
        ...
        const scheduledMs = (ride as any).scheduledFor ? new Date((ride as any).scheduledFor).getTime() : NaN;
  ...
  async function hydrateAuthUser(user: any) {
    ...
    const [liftClubMembership, operatorProfile] = await Promise.all([
      storage.getLiftClubMembershipByUser(user.id).catch(() => undefined),
      storage.getOperatorProfileByUserId(user.id).catch(() => undefined),
    ]);
    const partnerProfile = operatorProfile?.type === "partner"
      ? await storage.getPartnerProfileByOperatorId(operatorProfile.id).catch(() => undefined)
      : null;
  ...
  const PAY_LATER_DOCUMENT_TYPES = new Set([
    "pay_later:id_copy",
  ...
  async function getPayLaterApplication(userId: string) {
    const result = await pool.query(
    ...
    return {
      ...
      userEmail: app.user_username,
      userPhone: app.user_phone,
      status: app.status,
      creditLimit: Number(app.credit_limit || 0),
      availableCredit: Number(app.available_credit || 0),
  ...
  async function reservePayLaterCredit(userId: string, rideId: string, amount: number) {
    const client = await pool.connect();
    try {
  ...
  async function refundPayLaterCredit(userId: string, rideId: string, amount: number) {
    if (!(amount > 0)) return;
    const client = await pool.connect();
    try {
      await client.query("BEGIN");
  ...
  async function ensureDriverOperatorForChauffeur(userId: string) {
    ...
    let chauffeur = await storage.getChauffeurByUserId(userId).catch(() => undefined);
    if (!chauffeur) {
  ...
  function getEarningsWeekBounds(now = new Date()): {
    weekStart: Date;
    weekEnd: Date;
    periodLabel: string;
    ...
    const endStr = new Date(startLocal.getTime() + 7 * 24 * 60 * 60 * 1000).toLocaleDateString("en-US", options);
  ...
  app.get("/api/fleet/earnings-summary", requireAuth, async (req: AuthedRequest, res: Response) => {
    try {
      ...
      const dailyEarnings = (days || []).map((dateNum: any, i: number) => {
        const dayDate = dayDates[i] || new Date();
        const dayStart = new Date(dayDate);
        dayStart.setUTCHours(2, 0, 0, 0); // 4:00 AM SAST
        const dayEnd = new Date(dayStart.getTime() + 24 * 60 * 60 * 1000 - 1);

        const dayRides = allRides.filter((r: any) => {
        ...
        const gross = dayRides.reduce((sum: number, r: any) => sum + (parseFloat(r.price) || 0), 0);
        const comm = dayRides.reduce((sum: number, r: any) => {
          const fare = parseFloat(r.price) || 0;
          const rate = parseFloat(r.commissionRate) || 0.15;
          return sum + (fare * rate);
        }, 0);
  ...
  app.get("/api/fleet/rides", requireAuth, async (req: AuthedRequest, res: Response) => {
    try {
      let profile = await storage.getOperatorProfileByUserId(req.auth!.sub);
      if (!profile) {
        const chauffeur = await storage.getChauffeurByUserId(req.auth!.sub);
      ...
      const validChauffeurs = chauffeurProfiles.filter(Boolean) as any[];
      const chauffeurIdMap = new Map(validChauffeurs.map((c) => [c.chauffeurId, c]));

      const fleetRides = allRides.filter((r: any) => {
  ...
  app.post("/api/fleet/invites", requireAuth, async (req: AuthedRequest, res: Response) => {
    try {
      ...
      // Keep the sender name short so the invite always fits a single SMS (160 chars).
      ...
      function formatEmailError(err?: string | null): string | null {
        if (!err) return null;
      ...
      const emailStatus = emailResult.status === "sent"
        ...
        : emailResult.status === "pending_configuration"
          ...
          : emailResult.status === "skipped"
            ? "skipped"
            : "failed";
      const rawError = emailResult.status === "sent" || emailResult.status === "skipped"
        ? (smsResult.status === "sent" ? null : smsResult.error || null)
        : emailResult.error || null;
      const emailError = formatEmailError(rawError);
  ...
  app.post("/api/fleet/invites/:id/resend", requireAuth, async (req: AuthedRequest, res: Response) => {
    try {
      const profile = await storage.getOperatorProfileByUserId(req.auth!.sub);
      if (!profile) return res.status(404).json({ message: "Operator profile not found" });
      ...
      const updated = (await storage.updateFleetDriverInvite(invite.id, {
        ...
        emailError,
        resendId: emailResult.id || null,
        sentAt: emailResult.status === "sent" ? new Date() : invite.sentAt,
      })) || invite;
  ...
  // Admin-only: send a test SMS (and check provider status) to verify BulkSMS setup.
  app.get("/api/admin/sms-status", requireAuth, requireRole(["admin"]), async (_req: Request, res: Response) => {
    return res.json({ email: emailEnabled(), sms: smsEnabled() });
  });

  app.post("/api/admin/test-sms", requireAuth, requireRole(["admin"]), async (req: AuthedRequest, res: Response) => {
    try {
      const to = String(req.body?.to || "").trim();
      if (!to) return res.status(400).json({ message: "A phone number is required." });
      const driverAppUrl = process.env.DRIVER_APP_URL || "https://a2blift.com/driver";
      const message = String(req.body?.message || "").trim()
        || `A2B LIFT: A fleet partner invited you to drive for their fleet. Accept in the A2B driver app > Fleet: ${driverAppUrl}`;
  ...
  app.get("/api/admin/pay-later", requireAuth, requireRole(["admin"]), async (_req: AuthedRequest, res: Response) => {
    try {
      const result = await pool.query("SELECT user_id FROM pay_later_applications ORDER BY updated_at DESC");
  ...
  app.put("/api/admin/pay-later/:id", requireAuth, requireRole(["admin"]), async (req: AuthedRequest, res: Response) => {
    try {
      const status = String(req.body?.status || "").trim();
  ...
  app.post("/api/admin/pay-later/:id/credit", requireAuth, requireRole(["admin"]), async (req: AuthedRequest, res: Response) => {
    const client = await pool.connect();
    try {
      ...
      const application = result.rows[0];
      if (!application) { await client.query("ROLLBACK"); return res.status(404).json({ message: "Pay Later application not found." }); }
      if (application.status !== "approved") { await client.query("ROLLBACK"); return res.status(409).json({ message: "Approve the application before adding credit." }); }
      const before = Number(application.available_credit || 0);
      const after = Math.round((before + amount) * 100) / 100;
      ...
      await client.query("COMMIT");
      await storage.createNotification({ userId: application.user_id, title: "Pay Later credit added", body: `R${amount.toFixed(2)} was added. Available credit: R${after.toFixed(2)}.`, type: "pay_later" }).catch(() => undefined);
  ...
  app.delete("/api/admin/pay-later/:id/documents/:documentId", requireAuth, requireRole(["admin"]), async (req: AuthedRequest, res: Response) => {
    try {
      const result = await pool.query("DELETE FROM documents WHERE id = $1 AND pay_later_application_id = $2 RETURNING id", [req.params.documentId, req.params.id]);
  ...
  app.delete("/api/admin/pay-later/:id", requireAuth, requireRole(["admin"]), async (req: AuthedRequest, res: Response) => {
    try {
      const result = await pool.query("DELETE FROM pay_later_applications WHERE id = $1 RETURNING id", [req.params.id]);
      if (!result.rows[0]) return res.status(404).json({ message: "Pay Later application not found." });
      return res.json({ success: true });
    } catch (error: any) { return res.status(500).json({ message: error.message }); }
  });
  ...
  app.post("/api/rides", requireAuth, async (req: AuthedRequest, res: Response) => {
    try {
      ...
      // ── Reservations: book a ride in advance ──
      let scheduledFor: Date | null = null;
      if (rideData.scheduledFor) {
        const parsed = new Date(rideData.scheduledFor);
        const minLeadMs = 25 * 60 * 1000;
        const maxLeadMs = 30 * 24 * 60 * 60 * 1000;
      ...
      if (paymentMethod === "pay_later") {
        ...
        try {
          ...
          if (creditReserved) {
            await refundPayLaterCredit(clientId, ride.id, safeFare).catch(() => undefined);
          }
          await storage.updateRide(ride.id, { status: "cancelled", paymentStatus: "failed" }).catch(() => undefined);
  ...
  app.put("/api/rides/:id/stops", requireAuth, async (req: AuthedRequest, res: Response) => {
    try {
      ...
      const completedStopCount = Math.max(
        0,
        Math.min(previousStops.length, Number((existingRide as any).completedStopCount || 0)),
      );
      for (let index = 0; index < completedStopCount; index += 1) {
        ...
        if (
          !next ||
          Math.abs(previous.lat - next.lat) > 0.00001 ||
          Math.abs(previous.lng - next.lng) > 0.00001
        ) {
          return res.status(409).json({
      ...
      const payload = {
        ...updatedRide,
        stopsUpdatedAt: new Date().toISOString(),
      };
  ...
  app.put("/api/rides/:id/status", requireAuth, async (req: AuthedRequest, res: Response) => {
    try {
      ...
      if (status === "cancelled") {
        ...
        // 50% of the fare; the other 50% is refunded by the block below.
        const isReservation = (existingRide as any).scheduledFor && existingRide.status === "reserved";
        if (isReservation && cancelledBy === "client" && (existingRide as any).paymentStatus === "paid") {
      ...
      // Trip settlement performs several independent ledger and notification
      // writes. Acknowledge the authoritative ride completion first so the
      // driver's UI is never held open by those background operations.
      if (status === "trip_completed") {
      ...
      if (status === "cancelled" && rideBeforeUpdate) {
        try {
          ...
          // ── Pay Later → restore only the refundable amount to Pay Later credit ──
          const payLaterPayment = !cardPayment && !walletPayment
            ? payments.find((p: any) => p.method === "pay_later" && p.status === "paid")
            : null;
          if (payLaterPayment) {
  ...
  app.post("/api/rides/:id/cash-settlement", requireAuth, async (req: AuthedRequest, res: Response) => {
    try {
      const ride = await storage.getRide(req.params.id);
      if (!ride) return res.status(404).json({ message: "Ride not found" });
      ...
      if (difference !== 0 && ride.clientId) {
        ...
        if (rider) {
          ...
          if (difference > 0) {
            // Overpayment -> Credit change to rider wallet
            await storage.createWalletTransaction({
              userId: rider.id,
              type: "deposit",
              amount: difference,
            ...
            // Underpayment -> Deduct shortage from rider wallet (may go negative)
  ...
  app.get("/api/earnings/chauffeur/:chauffeurId/overview", async (req: Request, res: Response) => {
    try {
      const chauffeur = await storage.getChauffeur(req.params.chauffeurId);
      if (!chauffeur) return res.status(404).json({ message: "Chauffeur not found" });
      const user = chauffeur.userId ? await storage.getUser(chauffeur.userId) : null;

      const { weekStart, weekEnd, periodLabel, nextPayoutLabel } = getEarningsWeekBounds(new Date());

      const earningsList = await storage.getEarningsByChauffeur(chauffeur.id);
      const currentWeekEarnings = earningsList.filter((e: any) => {
        if (!e.createdAt) return false;
        const t = new Date(e.createdAt).getTime();
        return t >= weekStart.getTime() && t <= weekEnd.getTime();
      });

      const currentWeekAmount = currentWeekEarnings.reduce((sum: number, e: any) => sum + (Number(e.amount) || 0), 0);
      const walletBalance = Number(user?.walletBalance ?? (chauffeur as any)?.walletBalance ?? 0);
  ...
  app.get("/api/earnings/chauffeur/:chauffeurId/week-details", async (req: Request, res: Response) => {
    try {
      const chauffeur = await storage.getChauffeur(req.params.chauffeurId);
      if (!chauffeur) return res.status(404).json({ message: "Chauffeur not found" });
      const user = chauffeur.userId ? await storage.getUser(chauffeur.userId) : null;

      const targetDate = req.query.weekStart ? new Date(String(req.query.weekStart)) : new Date();
      const currentBounds = getEarningsWeekBounds(new Date());
      const bounds = getEarningsWeekBounds(isNaN(targetDate.getTime()) ? new Date() : targetDate);

      const [earningsList, allRides] = await Promise.all([
        storage.getEarningsByChauffeur(chauffeur.id),
        storage.getRidesByChauffeur(chauffeur.id),
      ]);

      const weekEarnings = earningsList.filter((e: any) => {
        if (!e.createdAt) return false;
        const t = new Date(e.createdAt).getTime();
      ...
      const dailyEarnings = bounds.days.map((dateNum, i) => {
        ...
        const dayEarns = weekEarnings.filter((e: any) => {
          const t = new Date(e.createdAt).getTime();
      ...
      const prevWeekDate = new Date(bounds.weekStart.getTime() - 2 * 24 * 60 * 60 * 1000);
      const prevBounds = getEarningsWeekBounds(prevWeekDate);

      const nextWeekDate = new Date(bounds.weekEnd.getTime() + 2 * 24 * 60 * 60 * 1000);
      ...
      const ratingPoints = Number(user?.rating || chauffeur?.rating || 5.0).toFixed(1);
  ...
  app.get("/api/earnings/chauffeur/:chauffeurId/weeks", async (req: Request, res: Response) => {
    try {
      const chauffeur = await storage.getChauffeur(req.params.chauffeurId);
      if (!chauffeur) return res.status(404).json({ message: "Chauffeur not found" });

      const earningsList = await storage.getEarningsByChauffeur(chauffeur.id);
      const createdAt = chauffeur.createdAt ? new Date(chauffeur.createdAt) : new Date(Date.now() - 60 * 24 * 60 * 60 * 1000);
      const now = new Date();

      const page = Math.max(1, parseInt(String(req.query.page || "1"), 10));
      const limit = Math.max(5, Math.min(52, parseInt(String(req.query.limit || "20"), 10)));

      // Generate all weeks backwards from current week to createdAt
      const allWeeks: any[] = [];
      ...
      while (safetyCounter < 156) {
        ...
        const wEarns = earningsList.filter((e: any) => {
          if (!e.createdAt) return false;
          const t = new Date(e.createdAt).getTime();
        ...
        cursorDate = new Date(b.weekStart.getTime() - 24 * 60 * 60 * 1000);
        safetyCounter++;
      }
  ...
  app.get("/api/earnings/chauffeur/:chauffeurId/activity", async (req: Request, res: Response) => {
    try {
      const chauffeur = await storage.getChauffeur(req.params.chauffeurId);
      if (!chauffeur) return res.status(404).json({ message: "Chauffeur not found" });
      ...
      const activities = weekRides.map((ride: any) => {
        const matchingEarning = earningsList.find((e: any) => e.rideId === ride.id);
      ...
      // Period string formatted e.g. "Aug 31 – Sep 6, 2026"
      const sastOffsetMs = 2 * 60 * 60 * 1000;
      const startLocal = new Date(bounds.weekStart.getTime() + sastOffsetMs);
      const endSunday = new Date(bounds.weekEnd.getTime() + sastOffsetMs - 4 * 60 * 60 * 1000);
  ...
  // ── Manual withdrawals: user requests → admin approves → admin pays via EFT ──

  /** Return held funds to the requester (used on reject/delete). */
  async function refundWithdrawalHold(withdrawal: any) {
  ...
  async function getWithdrawalRequesterUserId(withdrawal: any): Promise<string | null> {
    if (withdrawal?.userId) return withdrawal.userId;
    if (withdrawal?.chauffeurId) {
      const chauffeur = await storage.getChauffeur(withdrawal.chauffeurId).catch(() => undefined);
      return chauffeur?.userId || null;
    }
    return null;
  }

  // User (rider or driver) submits a withdrawal request for admin approval.
  app.post("/api/withdrawals", requireAuth, async (req: AuthedRequest, res: Response) => {
    try {
      ...
      // Prefer driver earnings; fall back to wallet balance (rider top-ups, commissions, refunds)
      let source: "driver_earnings" | "wallet";
      ...
      try {
        ...
        for (const admin of admins) {
          ...
          if ((admin as any).pushToken) {
            sendExpoPushNotification(
              [(admin as any).pushToken],
  ...
  app.get("/api/withdrawals/my", requireAuth, async (req: AuthedRequest, res: Response) => {
    try {
      const userId = req.auth!.sub;
      const chauffeur = await storage.getChauffeurByUserId(userId).catch(() => undefined);
      const all = await storage.getAllWithdrawals();
      const mine = (all || []).filter((w: any) =>
        w.userId === userId || (chauffeur && w.chauffeurId === chauffeur.id)
      );
  ...
  app.get("/api/withdrawals", requireAuth, requireRole(["admin"]), async (_req: Request, res: Response) => {
    try {
      const allWithdrawals = await storage.getAllWithdrawals();
      // Enrich with requester details for the admin dashboard
      const enriched = await Promise.all((allWithdrawals || []).map(async (w: any) => {
        try {
  ...
  // Admin: edit details, approve, decline, or mark paid.
  app.put("/api/withdrawals/:id", requireAuth, requireRole(["admin"]), async (req: AuthedRequest, res: Response) => {
    try {
      ...
      // Amount edits adjust the held funds by the difference
      if (req.body?.amount !== undefined) {
        ...
        if (delta !== 0 && ["pending", "approved"].includes(existing.status)) {
          if ((existing as any).source === "wallet" && (existing as any).userId) {
            const holder = await storage.getUser((existing as any).userId);
            if (holder) {
      ...
      const withdrawal = await storage.updateWithdrawal(req.params.id, updates as any);
      ...
      if (updates.status) {
        try {
          ...
          if (requesterUserId) {
            ...
            const copy = updates.status === "approved"
              ? `Your withdrawal of R${amt} was approved. The EFT payment is being processed.`
              : updates.status === "paid"
                ? `Your withdrawal of R${amt} has been paid via EFT to ${withdrawal.bankName}.`
                : updates.status === "rejected"
                  ? `Your withdrawal of R${amt} was declined. The funds have been returned to your balance.`
                  : `Your withdrawal of R${amt} was updated.`;
            await storage.createNotification({
            ...
            if ((requester as any)?.pushToken) {
              sendExpoPushNotification([(requester as any).pushToken], "Withdrawal update", copy, { type: "withdrawal" });
            }
          }
        } catch {}
      }
  ...
  function isAllowedPaystackReturnUrl(rawUrl: unknown): rawUrl is string {
  ...
  function appendPaystackReturnParams(rawUrl: string, params: Record<string, string>) {
    try {
      const url = new URL(rawUrl);
  ...
  app.get("/api/payments/webview-callback", (req: Request, res: Response) => {
    ...
    const defaultNativeReturnUrl = appVariant === "driver"
      ? "a2blift://payments/paystack-callback"
      : appVariant === "client"
        ? "a2bliftclient://payments/paystack-callback"
        : "";
    const requestedReturnUrl = req.query.returnUrl;
    const nativeReturnUrl = isAllowedPaystackReturnUrl(requestedReturnUrl)
      ? requestedReturnUrl
      : defaultNativeReturnUrl;
    const nativeAppUrl = nativeReturnUrl
      ? appendPaystackReturnParams(nativeReturnUrl, { reference, status })
      : "";
    const webFallbackUrl = process.env.FRONTEND_URL
      || getAppBaseUrl(req)
      || "https://a2blift.com";
...
<body>
  ...
  app.post("/api/payments/pay-wallet", requireAuth, async (req: AuthedRequest, res: Response) => {
    try {
      ...
      // Spendable balance = wallet (commissions/top-ups) + rewards (referrals etc.)
      const walletBefore = Number(user.walletBalance || 0);
      const rewardsBefore = Number((user as any).rewardsBalance || 0);
      ...
      if (fromWallet > 0) {
        await recordWalletTx(userId, "ride_charge", fromWallet, walletBefore, "Ride payment", undefined, rideId);
      }
      if (fromRewards > 0) {
        await storage.createNotification({
          userId,
  ...
  app.delete("/api/admin/withdrawals/:id", requireAuth, requireRole(["admin"]), async (req: AuthedRequest, res: Response) => {
    try {
      const existing = await storage.getWithdrawal(req.params.id);
      if (!existing) return res.status(404).json({ message: "Withdrawal not found" });
      // Return held funds unless the withdrawal was already paid out or refunded
